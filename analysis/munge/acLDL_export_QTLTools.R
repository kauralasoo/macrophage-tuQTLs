...
se_revised = readRDS("results/SummarizedExperiments/acLDL_salmon_txrevise_promoters.rds")
event_metadata = rowData(se_revised)
unique_genes = unique(event_metadata$gene_id)
...
fastqtl_genepos = tbl_df2(rowData(event_dataset)) %>% 
  dplyr::filter(transcript_id %in% rownames(event_dataset)) %>%
  constructQTLtoolsGenePos()
output_path = "processed/acLDL/qtltools/input/txrevise_promoters/"
...
#Export txrevise end counts for QTLtools
se_revised = readRDS("results/SummarizedExperiments/acLDL_salmon_txrevise_ends.rds")
event_metadata = rowData(se_revised)
unique_genes = unique(event_metadata$gene_id)
...
fastqtl_genepos = tbl_df2(rowData(event_dataset)) %>% 
  dplyr::filter(transcript_id %in% rownames(event_dataset)) %>%
  constructQTLtoolsGenePos()
output_path = "processed/acLDL/qtltools/input/txrevise_ends/"
...
se_gene = readRDS("results/SummarizedExperiments/acLDL_featureCounts.rds")
event_metadata = rowData(se_gene)
unique_genes = unique(event_metadata$gene_id)

#Identify expressed genes from Salmon TPMs
salmon_se = readRDS("results/SummarizedExperiments/acLDL_salmon_gene_abundances.rds")
...
fastqtl_genepos = tbl_df2(rowData(expressed_dataset)) %>% 
  ...
  dplyr::mutate(transcript_id = gene_id) %>%
  constructQTLtoolsGenePos()
output_path = "processed/acLDL/qtltools/input/featureCounts/"
...
#Calculate fold-change
sample_list = colnames(event_conditions_renamed$Ctrl)
fc_list = list(Diff = cqn_list$AcLDL[,sample_list] -
                 cqn_list$Ctrl[,sample_list])
qtltools_list = purrr::map(fc_list, prepareQTLtoolsMatrix, fastqtl_genepos)
saveFastqtlMatrices(qtltools_list, output_path, file_suffix = "norm_prop")
